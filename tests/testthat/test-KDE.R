...
test_that("epanechnikov_kernel returns correct values", {
  expect_equal(epanechnikov_kernel(c(0,1,10,0.5)), c(0.75,0,0,0.5625))
  expect_equal(epanechnikov_kernel_cpp(c(0,1,10,0.5)), c(0.75,0,0,0.5625))
})

test_that("gaussian_kernel returns correct values", {
  expect_equal(gaussian_kernel(0), 1 / sqrt(2 * pi))
  expect_equal(gaussian_kernel(1), (1 / sqrt(2 * pi)) * exp(-0.5))
})
...
test_that("kernel evaluators returns correct density estimates", {
  ...
  result <- kernel_vec(grid, vec_obs, bandwidth = 0.5, epanechnikov_kernel_cpp)
  result_cpp <- kernel_cpp_epanechnikov(grid, vec_obs, bandwidth = 0.5)
...
test_that("kernel_density_estimation computes density estimates", {
  ...
  result <- kernel_density_estimation(vec_obs, bandwidth = 1, kernel_calculator = kernel_vec, kernel_function = gaussian_kernel)
...
test_that("bandwidth_silvermans_rule calculates bandwidth correctly", {
  vec_obs <- c(1, 2, 3, 4, 5)
  bw <- bandwidth_silvermans_rule(vec_obs)
...
test_that("get plug-in is correct", {
  ...
  bw <- calculate_oracle_bw_epanechnikov(vec_obs = norm_data)
...
test_that("get bw silverman is correct", {
  ...
  bw <- bandwidth_silvermans_rule(norm_data)
...
test_that("binning_gives_correct_results", {
  ...
  bin_Vals_cpp <- binning_cpp(vec, lower = min(vec), upper = max(vec), number_of_bins = n)
